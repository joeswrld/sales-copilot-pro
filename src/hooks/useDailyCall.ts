...
export function useLiveCall(options?: {
  ...
  const endCall = useMutation({
    mutationFn: async () => {
      ...
      //
      // FIX: this used to call generate-call-summary directly, which reads
      // whatever is in `transcripts` the instant it's invoked — right as
      // the host clicks "End Call". At that exact moment the last few
      // seconds of speech can still be mid-flight through transcribe-live
      // (WS relay, speech_final not yet fired) or the chunked fallback
      // pipelines (still uploading their last buffer), so the closing
      // seconds of the call could be permanently missed from the summary,
      // score, and coaching. flush-and-finalize-call waits for the
      // transcript row count to settle (bounded, ~12s max) before calling
      // generate-call-summary itself, so this is a strict superset of the
      // old behavior — same result, just no longer racing the tail of the
      // conversation.
