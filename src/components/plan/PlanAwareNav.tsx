 ...
 * Import and use alongside your existing DashboardLayout nav.
 */
...
export interface NavItemWithGate {
  ...
  feature?: PlanFeatureKey; // if set, gated
}
...
export function SidebarPlanFooter() {
  const navigate = useNavigate();
  const { planKey, planName, usagePct, minutesUsed, minuteLimit, isUnlimited, planIndex } = usePlanEnforcement();

  const colors: Record<string, string> = {
    free: "rgba(255,255,255,.3)", starter: "#60a5fa", growth: "#0ef5d4", scale: "#a78bfa",
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: isUnlimited ? 0 : 6 }}>
      ...
      {!isUnlimited && (
        <>
          <div style={{ height: 3, borderRadius: 2, background: "rgba(255,255,255,.08)", overflow: "hidden", marginBottom: 4 }}>
            <div style={{
              height: "100%", borderRadius: 2,
              width: `${Math.min(usagePct, 100)}%`,
              background: usagePct >= 80 ? "#f59e0b" : usagePct >= 100 ? "#ef4444" : color,
