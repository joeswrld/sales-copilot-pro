...
import {
  ...
  MapPin, Briefcase, RefreshCw, UserCheck, Sparkles,
} from "lucide-react";
...
function SemanticResultRow({ result, onClick }: { result: SemanticSearchResult; onClick: () => void }) {
  const pct = Math.round(result.similarity * 100);
  const color = pct >= 70 ? "#16a34a" : pct >= 50 ? "#b45309" : "rgba(23,23,15,0.4)";
  const bg = pct >= 70 ? "rgba(34,197,94,0.12)" : pct >= 50 ? "rgba(251,191,36,0.15)" : "rgba(23,23,15,0.06)";
  return (
    <div
      onClick={onClick}
      style={{
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11.5, color: "rgba(23,23,15,0.4)", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          {result.candidate_current_role && (
            <span style={{ display: "flex", alignItems: "center", gap: 3 }}>
              <Briefcase style={{ width: 10, height: 10 }} />{result.candidate_current_role}
              {result.current_company ? ` · ${result.current_company}` : ""}
            </span>
          )}
          {result.location && (
            <span style={{ display: "flex", alignItems: "center", gap: 3 }}>
              <MapPin style={{ width: 10, height: 10 }} />{result.location}
            </span>
          )}
          {result.years_experience != null && <span>{result.years_experience}y exp</span>}
        </div>
      </div>
      <div style={{
        fontSize: 10.5, fontWeight: 700, padding: "4px 9px", borderRadius: 8, flexShrink: 0,
        background: bg, color,
      }}>
        {pct}% match
      </div>
...
export default function CandidatesPage() {
  ...
  // AI (semantic) search is a distinct mode from the plain-text filter above
  // rather than a replacement for it — the plain filter is instant,
  // client-side, and great for "find Amara" by name; semantic search is a
  // natural-language query against candidates.profile_embedding via
  // parse-candidate-cv's embed_query path + semantic_search_candidates.
  // Switching modes doesn't clear the other's input, so toggling back and
  ...
  const runAiSearch = async () => {
    if (!teamId || !aiQuery.trim()) return;
    setAiSearching(true);
    setAiError(null);
    try {
      // Step 1: turn the natural-language query into a 768-dim embedding.
      // This never touches candidate data — parse-candidate-cv's embed_query
      // path only ever returns a vector for our own subsequent call below.
      const { data: embedData, error: embedErr } = await supabase.functions.invoke("parse-candidate-cv", {
        body: { mode: "embed_query", query_text: aiQuery.trim() },
      });
      if (embedErr || (embedData as any)?.error) {
        throw new Error((embedData as any)?.error || embedErr?.message || "Failed to process search query");
      }
      const embedding = (embedData as any)?.embedding;
      if (!Array.isArray(embedding)) throw new Error("Search query could not be processed");

      // Step 2: semantic_search_candidates re-checks team membership itself
      // and ranks the team's candidates by cosine similarity to the query.
      const { data: results, error: searchErr } = await (supabase as any).rpc("semantic_search_candidates", {
        p_team_id: teamId,
        p_query_embedding: embedding,
        p_limit: 20,
      });
      if (searchErr) throw searchErr;
      setAiResults((results ?? []) as SemanticSearchResult[]);
    } catch (e: any) {
  ...
  return (
    <DashboardLayout>
      <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 900, margin: "0 auto", fontFamily: "'Inter', sans-serif" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              style={{
                ...
                background: aiMode ? "linear-gradient(135deg, #22315C, #2A3F73)" : "#fff",
                border: aiMode ? "none" : "1px solid rgba(23,23,15,0.15)", borderRadius: 12,
                color: aiMode ? "#FAFAF8" : "#17170F", fontSize: 13, fontWeight: 700, cursor: "pointer",
              }}
            >
              <Sparkles style={{ width: 14, height: 14 }} />AI Search
            </button>
            <button
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ position: "relative", flex: 1 }}>
                <Sparkles style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", width: 13, height: 13, color: "#22315C" }} />
                <input
                  value={aiQuery}
                  onChange={e => setAiQuery(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter") runAiSearch(); }}
                  placeholder="Senior React developers in London with 5+ years, available within 30 days…"
                  style={{ width: "100%", paddingLeft: 32, paddingRight: 12, paddingTop: 10, paddingBottom: 10, background: "rgba(34,49,92,0.05)", border: "1px solid rgba(34,49,92,0.22)", borderRadius: 10, color: "#17170F", fontSize: 13, outline: "none", boxSizing: "border-box" }}
                />
              </div>
              <button
                onClick={runAiSearch}
                disabled={aiSearching || !aiQuery.trim()}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", background: "#22315C", border: "none", borderRadius: 10, color: "#fff", fontSize: 13, fontWeight: 700, cursor: aiSearching || !aiQuery.trim() ? "not-allowed" : "pointer", opacity: aiSearching || !aiQuery.trim() ? 0.5 : 1, flexShrink: 0 }}
              >
                {aiSearching ? <Loader2 style={{ width: 14, height: 14, animation: "spin 1s linear infinite" }} /> : "Search"}
              </button>
            </div>

            {aiError && (
              <div style={{ fontSize: 12.5, color: "#b3432f", background: "rgba(179,67,47,0.08)", border: "1px solid rgba(179,67,47,0.22)", borderRadius: 10, padding: "10px 12px" }}>
                {aiError}
              </div>
            )}
            ...
            {aiResults !== null && !aiError && (
              aiResults.length === 0 ? (
                <div style={{ textAlign: "center", padding: "32px 20px", color: "rgba(23,23,15,0.35)", fontSize: 13 }}>
                  No candidates matched that description.
                </div>
              ) : (
                <div style={{ background: "rgba(23,23,15,0.02)", border: "1px solid rgba(23,23,15,0.06)", borderRadius: 14, overflow: "hidden" }}>
                  {aiResults.map(r => (
                    <SemanticResultRow key={r.candidate_id} result={r} onClick={() => navigate(`/candidates/${r.candidate_id}`)} />
                  ))}
                </div>
              )
            )}
        ...
        {aiMode ? null : loading || teamLoading ? (
