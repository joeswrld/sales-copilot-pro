...
class AudioChunkProcessor {
  ...
  start() {
    ...
    this.intervalId = window.setInterval(() => {
      if (this.mediaRecorder?.state === "recording") { this.mediaRecorder.stop(); this.mediaRecorder.start(); }
    }, this.intervalMs);
  }
...
// ─── Hook: HMS room ────────────────────────────────────────────────────────────
function useHMSRoom() {
  const [roomInfo, setRoomInfo] = useState<{ room_id: string; room_name: string; share_link: string; mgmt_token: string; } | null>(null);
  ...
  const createRoom = useCallback(async (callId: string, title: string, description?: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("create-hms-room", {
        ...
        body: { call_id: callId, title, description: description || null, app_origin: window.location.origin },
      });
...
function ScheduleModal({
  ...
  prefillLink?: string;
  prefillTitle?: string;
  onSave: (params: { title: string; meeting_link: string; scheduled_time: string; meeting_type: string }) => Promise<void>;
  onClose: () => void;
}) {
...
function ZombieBanner({ callId, onCleared }: { callId: string | null | undefined; onCleared: () => void }) {
  ...
  const doClear = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("force-clear-session", {
        headers, body: { call_id: callId ?? null, clear_all: true },
      });
      if (!error && data?.success) success = true;
    } catch {}
    if (!success) {
      try {
        const { error } = await supabase.from("calls").update({ status: "completed", end_time: new Date().toISOString(), duration_minutes: 0 }).eq("status", "live");
  ...
  return (
    <div className={cn("rounded-xl border p-4", clearState === "done" ? "border-green-500/30 bg-green-500/5" : clearState === "failed" ? "border-red-500/30 bg-red-500/5" : "border-yellow-500/30 bg-yellow-500/5")}>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className={cn("text-sm font-semibold", clearState === "done" ? "text-green-400" : clearState === "failed" ? "text-red-400" : "text-yellow-400")}>
        ...
        {(clearState === "idle" || clearState === "failed") && (
          <div className="flex gap-2">
            <Button size="sm" variant="outline" className={cn("gap-1.5 h-8 text-xs", clearState === "failed" ? "border-red-500/30 text-red-400" : "border-yellow-500/30 text-yellow-400")} onClick={doClear} disabled={clearState === "clearing"}>
...
export default function LiveCall() {
  ...
  // Zombie detection
  useEffect(() => {
    if (isLive && liveCall && !(liveCall as any).meeting_url && !roomInfo && !hostJoined) setIsZombie(true);
    else if (!isLive) setIsZombie(false);
  ...
  const handleJoinAsHost = useCallback(async (info?: { room_id: string; room_name: string; share_link: string; mgmt_token: string }) => {
  ...
  const handleCreateMeeting = useCallback(async () => {
    ...
    try {
      ...
      const info = await createRoom(callRow.id, title, meetingNotes);
      setShowPopup(true);
      toast.success("Room created! Share the link with your prospect.");
      return info;
    } catch (err: any) {
      if (callRow?.id) {
        await supabase.from("calls").update({ status: "completed", end_time: new Date().toISOString(), duration_minutes: 0 }).eq("id", callRow.id).catch(() => {});
      }
      if (err?.message === "PLAN_LIMIT_REACHED") toast.error("Meeting limit reached. Upgrade to continue.");
      else toast.error("Could not create meeting room.");
  ...
  const handleHostFromLink = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Could not host from this link");
    }
  }, [joinLink]);
  ...
  const handleEndCall = useCallback(async () => {
    ...
    try {
      if (hmsActionsRef.current) { try { await hmsActionsRef.current.leave(); } catch {} }
    ...
    <DashboardLayout>
      ...
      <div className="space-y-5 pb-10">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Meeting control center — create, join, schedule, and manage calls</p>
        </div>
        ...
        {hasActiveSession && hostJoined && (
          <div className="rounded-xl border border-green-500/25 bg-green-500/5 p-4 flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={handleToggleAudio} className={cn("flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors", isAudioOn ? "border-border bg-secondary/60" : "border-red-500/30 bg-red-500/15 text-red-400")}>
                {isAudioOn ? <Mic className="w-3.5 h-3.5" /> : <Mic className="w-3.5 h-3.5" />}{isAudioOn ? "Mute" : "Unmute"}
              </button>
              <button onClick={handleToggleVideo} className={cn("flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors", isVideoOn ? "border-border bg-secondary/60" : "border-red-500/30 bg-red-500/15 text-red-400")}>
                {isVideoOn ? <Video className="w-3.5 h-3.5" /> : <VideoOff className="w-3.5 h-3.5" />}{isVideoOn ? "Stop Video" : "Start Video"}
              </button>
              ...
              <Button size="sm" variant="destructive" className="gap-1.5 h-8 text-xs" onClick={handleEndCall} disabled={endCall.isPending}>
                {endCall.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <PhoneOff className="w-3 h-3" />}End Call
              </Button>
            </div>
          </div>
        )}
        ...
        {roomInfo && !hostJoined && (
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" className="gap-1.5 h-8 text-xs" onClick={() => handleJoinAsHost()}>
            ...
            <div className="glass rounded-xl border border-border p-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {MEETING_TYPES.map(t => (
                    <button key={t.value} onClick={() => setMeetingType(t.value)}
                      className={cn("text-[11px] px-2 py-1.5 rounded-lg border transition-all text-center leading-tight",
                        meetingType === t.value ? "border-primary/50 bg-primary/10 text-foreground" : "border-border bg-secondary/30 text-muted-foreground hover:bg-secondary/60"
                      )}>
                      <div>{t.emoji}</div>
              ...
              <div>
                ...
                <textarea value={meetingNotes} onChange={e => setMeetingNotes(e.target.value)} rows={2} placeholder="Agenda, context…"
          ...
          <div className="space-y-4">
            <div className="glass rounded-xl border border-border p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant="outline" className="gap-1.5" onClick={handleJoinFromLink} disabled={isJoining || !joinLink.trim()}>
                  <ExternalLink className="w-4 h-4" />Join Meeting
                </Button>
                <Button className="gap-1.5" onClick={handleHostFromLink} disabled={isJoining || !joinLink.trim()}>
                  <Video className="w-4 h-4" />Host Meeting
                </Button>
              </div>

              <div className="rounded-lg p-3 space-y-1.5" style={{ background: "rgba(99,102,241,0.05)", border: "1px solid rgba(99,102,241,0.1)" }}>
                <p className="text-xs font-medium text-indigo-400/80">How it works</p>
                <ul className="space-y-1">
                  {["Paste your 100ms meeting link above", "Join as guest or host with full controls", "AI transcription starts automatically"].map(tip => (
                    <li key={tip} className="flex items-center gap-2 text-xs text-muted-foreground">
                      <span className="w-1 h-1 rounded-full bg-indigo-400/50 shrink-0" />{tip}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {roomInfo && (
              <div className="glass rounded-xl border border-border p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" className="gap-1.5 flex-1 h-8 text-xs" onClick={() => handleJoinAsHost()}>
                    <Video className="w-3 h-3" />Join as Host
                  </Button>
                </div>
              </div>
            ...
            <div className="glass rounded-xl border border-border p-4 space-y-3">
              ...
              <div className="space-y-1">
                {[
                  ...
                  <Link key={to} to={to} className="flex items-center justify-between px-3 py-2 rounded-lg text-xs text-muted-foreground hover:text-foreground hover:bg-secondary/40 transition-colors">
